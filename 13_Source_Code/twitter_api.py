...
class Listener(tweepy.Stream):
    ...
    def on_status(self, status):
        if not status.truncated:
            ...
            res = predict(preprocess("", text))
            res = "Real" if res == 1 else "Not Real"
            self.tweets.append([status.user.screen_name, status.created_at, status.text, sentimentPrediction(text), res])

        else:
            text = status.user.screen_name + ". " + status.extended_tweet['full_text']
            res = predict(preprocess("", text))
            res = "Real" if res == 1 else "Not Real"
            self.tweets.append([status.user.screen_name, status.created_at, status.extended_tweet['full_text'], sentimentPrediction(text), res])
